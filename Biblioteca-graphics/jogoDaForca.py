...
desenhaForca(win)
n = 6
alfabeto = [chr(val) for val in range(ord("A"), ord("Z")+1)]
ganhou = False
cont = 0
k = 0
x = 280
...
while True:
    ...
    if cont == n:
        time.sleep(1.5)
        imag = Image(Point(
            500, 300), "C:\\Users\\seven\\Documents\\Projetos\\FURG\\AULA\\algortimo\\Graphics\\src\\121-1211555_sangre-cartoon-blood-splatter-png.gif")
        imag.draw(win)
        break
    letter = win.getKey().upper()
    while letter in usadas:
        letter = win.getKey().upper()
...
if ganhou:
    ...
    congr.draw(win)
    img = Image(Point(
        900, 300), "C:\\Users\\seven\\Documents\\Projetos\\FURG\\AULA\\algortimo\\Graphics\\src\\GUNzxe2b_400x400.gif")
    img.draw(win)
else:
    ...
    congr.draw(win)
    img = Image(Point(
        900, 300), "C:\\Users\\seven\\Documents\\Projetos\\FURG\\AULA\\algortimo\\Graphics\\src\\FOY_l23XwAgYutd.gif")
    img.draw(win)
